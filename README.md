...
| Документ | Описание |
|----------|---------|
| [📋 PRD v2.1](IndoorNav/PRD.md) | Требования к продукту: роли, функциональные требования, архитектура, user flows |
...
- **Поворот карты** на произвольный угол (ЛКМ + ПКМ); подписи и иконки узлов автоматически контр-ротируются — всегда читаемы
- **Ограничение зума** по зданиям: Верхний корпус — 0.7×, Нижний корпус — 0.88×
- **Ограничение панирования**: карта не может уйти за пределы экрана со стороны (гор. ≥†20% экрана остаётся под картой, верт. ≥†40% карты)
...
- График отправляется на локальный **ASP.NET Core сервер** (`Tools/EmergencyServer`, порт 5180); приложения опрашивают его каждые 30 с. и синхронизируют состояние
- Активация ЧС запускает **полноэкранный оверлей** (мигает каждые 4 с.) до подтверждения пользователем
...
- **Push-уведомления об эвакуации** на телефон: каждые 3 с (приложение в фоне) / 5 с (на переднем плане), пока ЧС активна
- **Диалог запроса разрешения на уведомления** — появляется после прохождения обучения, с пояснением «никаких других уведомлений не будет»
- Системный запрос разрешения (Android 13+ `POST_NOTIFICATIONS`, iOS `RequestNotificationPermission`)
...
### Обучение (Tutorial)
- При первом запуске — пошаговый туториал по интерфейсу (6 шагов)
- Подсветка элементов с жёлтой рамкой + стрелки + описание
- Индикатор шага и кнопка «Пропустить»
- После завершения обучения — диалог запроса разрешения на уведомления
...
IndoorNav/
├── Controls/
│   └── SvgView.cs                     # Контрол карты (SkiaSharp, VSync-рендер, иконки, вращение, "Вы тут")
├── Models/
...
│   ├── TutorialStep.cs                # Модель шага туториала
...
│   ├── EmergencyService.cs            # Режим ЧС + polling сервера (30 с.)
│   ├── ScheduleService.cs             # Расписание пар
│   ├── DepartmentService.cs           # Кафедры и группы
│   ├── QrService.cs                   # Генерация + декодирование QR (ZXing)
│   ├── NotificationService.cs         # Push-уведомления ЧС (Plugin.LocalNotification)
...
Tools/EmergencyServer/                 # ASP.NET Core Minimal API: сервер ЧС-оповещений (порт 5180)
...
| [Plugin.LocalNotification](https://github.com/thudugala/Plugin.LocalNotification) | 11.1.0 | Локальные push-уведомления (ЧС) |
